...
from database import Base,msg_history,User,room_guide
import random
import string
...
def continuechat(room):
    ...
    connected_user = session.query(User).filter_by(id=userinfo["user_id"]).first()
    room_guide1=session.query(room_guide).filter_by(room=room).first()
    return render_template('chat1.html',msgs=msgs,userinfo=login_session['profile'],
    	room=room,other_enduser=other_enduser,send_img="send_img",user_id=userinfo['user_id'],connected_user=connected_user,guide_id=room_guide1.guide_id)
...
@app.route('/begin_chat/<guide_id>')
@requires_auth
def begin_chat(guide_id):
    guide_id=guide_id
    userinfo=login_session['profile']
    DBSession = sessionmaker(bind=engine)
    session = DBSession()
